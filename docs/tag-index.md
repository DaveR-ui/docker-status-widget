# Tag Index — generated tag surface

Tag → document, for when a question arrives in vocabulary the tree does not obviously route. Written
by `node docs/validate.js --write`; regenerate rather than edit.

<!-- BEGIN GENERATED: tags -->
<!-- snapshot: 2026-09-11 -->
## Generated tag index

- **adr** — adr-0001-corpus-location-and-entry-point, adr-0002-lowercase-naming-convention, adr-0003-slice-taxonomy, adr-0004-scoped-polkit-grant, adr-0005-doc-classes-and-body-shape-exceptions, adr-0006-video-downloader-command-boundary, adr-0007-stop-button-confirmation-and-narrow-grant, adr-index, adr-template
- **bootstrap** — bootstrap-checklist
- **brownfield** — adr-0003-slice-taxonomy, slice-generalities-template
- **catalog** — interface-surface-template
- **checklist** — bootstrap-checklist, new-note-checklist
- **cli** — installer-index, installer-slice
- **command-boundary** — adr-0006-video-downloader-command-boundary
- **components** — ui-inventory-template
- **contract** — adr-0005-doc-classes-and-body-shape-exceptions, document-template, interface-surface-template, new-note-checklist
- **convention** — adr-0002-lowercase-naming-convention
- **daemon** — adr-0007-stop-button-confirmation-and-narrow-grant
- **data-engine** — data-engine-contract
- **decision-record** — adr-template
- **decisions** — adr-index
- **doc-classes** — adr-0005-doc-classes-and-body-shape-exceptions
- **documentation** — adr-0001-corpus-location-and-entry-point, readme
- **entry-point** — adr-0001-corpus-location-and-entry-point, project, project-md-template, readme
- **evidence** — verification-index, verification-slice
- **failures** — troubleshooting
- **generality** — installer-slice, privilege-slice, status-slice, verification-slice, widget-slice
- **hub** — adr-index, hub-template, installer-index, privilege-index, status-index, verification-index, widget-index
- **index** — adr-index, installer-index, privilege-index, readme, status-index, verification-index, widget-index
- **installer** — installer-index, installer-slice
- **interface-contract** — data-engine-contract
- **interface-surface** — data-engine-contract, interface-surface-template
- **inventory** — ui-inventory-template, widget-ui-surfaces
- **logic** — status-index, status-slice
- **naming** — adr-0002-lowercase-naming-convention
- **navigation** — hub-template, readme
- **packaging** — installer-slice
- **polkit** — adr-0004-scoped-polkit-grant, adr-0007-stop-button-confirmation-and-narrow-grant, privilege-index, privilege-slice
- **privilege** — adr-0004-scoped-polkit-grant, adr-0007-stop-button-confirmation-and-narrow-grant, privilege-index, privilege-slice
- **project** — project
- **project-md** — project-md-template
- **qml** — widget-index, widget-slice, widget-ui-surfaces
- **routing** — adr-0003-slice-taxonomy, project
- **schema** — adr-0005-doc-classes-and-body-shape-exceptions
- **security** — adr-0004-scoped-polkit-grant, adr-0006-video-downloader-command-boundary, adr-0007-stop-button-confirmation-and-narrow-grant, privilege-slice
- **slice** — installer-slice, privilege-slice, slice-generalities-template, status-slice, verification-slice, widget-slice
- **slices** — adr-0003-slice-taxonomy, project
- **status** — data-engine-contract, status-index, status-slice
- **structure** — adr-0001-corpus-location-and-entry-point
- **styles** — ui-inventory-template
- **surfaces** — widget-ui-surfaces
- **symptoms** — troubleshooting, troubleshooting-template
- **template** — adr-template, document-template, hub-template, interface-surface-template, project-md-template, slice-generalities-template, troubleshooting-template, ui-inventory-template
- **tests** — verification-index, verification-slice
- **tooling** — adr-0002-lowercase-naming-convention
- **troubleshooting** — troubleshooting, troubleshooting-template
- **ui** — adr-0007-stop-button-confirmation-and-narrow-grant, ui-inventory-template, widget-ui-surfaces
- **validator** — adr-0002-lowercase-naming-convention
- **verification** — troubleshooting, verification-index, verification-slice
- **video-download** — adr-0006-video-downloader-command-boundary
- **widget** — widget-index, widget-slice, widget-ui-surfaces
- **yt-dlp** — adr-0006-video-downloader-command-boundary
<!-- END GENERATED: tags -->
